class CLI
  PETS = ["Cat1", "Cat2", "Cat3", "Cat4", "Caat5", "Cat6", "Cat7", "Cat8"]

  def call
    puts "\nWELCOME TO CAT FINDER!".light_blue
    puts "--------------------------------"
    start
  end

  def start
    puts "What kind of Cat would you like to find today? (Please enter a number)\n".cyan
  ...
  def display_all_animals
    if @animal_object == Small
      puts "\n\nAVAILABLE SMALL & FURRY CATS FOR ADOPTION:".light_blue
    else
  ...
  def repeat?
    puts "\n\nWould you like to search more adoptable cats? (Please enter a number)\n".cyan
